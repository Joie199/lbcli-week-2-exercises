# Create a raw transaction with an amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
raw_tx=""
TXID=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.txid')
CHANGE_ADDRESS=$(bitcoin-cli -regtest getrawchangeaddress)
RAW_TX=$(bitcoin-cli -regtest createrawtransaction '[{"txid":"23c19f37d4e92e9a115aab86e4edc1b92a51add4e0ed0034bb166314dde50e16","vout":0},{"txid":"23c19f37d4e92e9a115aab86e4edc1b92a51add4e0ed0034bb166314dde50e16","vout":1}]' '{"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP":0.20000000}')
echo "$RAW_TX"
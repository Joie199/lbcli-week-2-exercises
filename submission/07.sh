# Create a raw transaction with an amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
raw_tx=""
bitcoin-cli -regtest decoderawtransaction "$raw_tx"

bitcoin-cli -regtest getrawtransaction 2577efbd235f00d1d92c77e3e4b2a5f6d9954547d0865f8d695eecbbde8e92b0c8 true

change=$(bitcoin-cli -regtest getrawchangeaddress)

unsigned_tx=$(bitcoin-cli -regtest createrawtransaction \
'[{"txid":"2577efbd235f00d1d92c77e3e4b2a5f6d9954547d0865f8d695eecbbde8e92b0c8","vout":0}]' \
"{\"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP\":0.20000000,\"$change\":0.03678108}")

bitcoin-cli -regtest decoderawtransaction "$unsigned_tx"

signed_tx=$(bitcoin-cli -regtest signrawtransactionwithwallet "$unsigned_tx")

signed_hex=$(echo "$signed_tx" | jq -r '.hex')

bitcoin-cli -regtest decoderawtransaction "$signed_hex"

bitcoin-cli -regtest sendrawtransaction "$signed_hex"
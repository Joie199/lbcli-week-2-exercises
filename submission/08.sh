# Create a raw transaction with a transaction fee of 20,000 satoshis 
# The transaction fee should be 20,000 satoshis, then the rest of the amount should be sent to the address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP
# Use the UTXOs from the transaction below
raw_tx=""
recipient="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
vin_txid=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.vin[0].txid')
vout0=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.vin[0].vout')
vout1=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.vin[1].vout')
rawtxhex=$(bitcoin-cli -regtest createrawtransaction '''[ { "txid": "'$txid'", "vout0": 0,  "sequence": 4294967293 }, { "txid": "'$txid'", "vout1": 1, "sequence": 4294967293 } ]''' '''{ "'$recipient'": 0.023 }''')

echo $rawtxhex
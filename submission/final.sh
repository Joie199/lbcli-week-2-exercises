...
VERIFY_PAYMENT_SATS=$(echo "$DECODED_TX" | jq -r --arg addr "$PAYMENT_ADDRESS" '
  .vout
  | map(select((.scriptPubKey.address // .scriptPubKey.addresses[0]) == $addr))
  | .[0].value
  | (. * 100000000 | round)
')
check_cmd "Payment verification" "VERIFY_PAYMENT_SATS" "$VERIFY_PAYMENT_SATS"

VERIFY_CHANGE_SATS=$(echo "$DECODED_TX" | jq -r --arg addr "$CHANGE_ADDRESS" '
  .vout
  | map(select((.scriptPubKey.address // .scriptPubKey.addresses[0]) == $addr))
  | .[0].value
  | (. * 100000000 | round)
')
check_cmd "Change verification" "VERIFY_CHANGE_SATS" "$VERIFY_CHANGE_SATS"

echo "Verification Results:"
echo "- RBF enabled: $VERIFY_RBF"
echo "- Payment to $PAYMENT_ADDRESS with amount $VERIFY_PAYMENT_SATS sats"
echo "- Change to $CHANGE_ADDRESS with amount $VERIFY_CHANGE_SATS sats"

# Final verification
if [ "$VERIFY_RBF" = "true" ] && [ "$VERIFY_PAYMENT_SATS" -eq "$PAYMENT_AMOUNT" ] && [ "$VERIFY_CHANGE_SATS" -eq "$CHANGE_AMOUNT" ]; then
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(echo "$DECODED_TX" | jq -r '.vout | to_entries[] | select((.value.scriptPubKey.address // .value.scriptPubKey.addresses[0])=="'"$CHANGE_ADDRESS"'") | .key')
...
# STUDENT TASK: Create the outputs JSON structure
CHILD_OUTPUTS=$(jq -n --arg addr "$CHILD_RECIPIENT" --argjson amt "$CHILD_SEND_BTC" '{($addr): $amt}')
